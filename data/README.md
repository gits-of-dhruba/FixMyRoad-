...
python load_roads.py --db roadwatch --user postgres --password yourpassword --csv bengaluru_roads_master.csv
```
...
```bash
python load_roads.py --db roadwatch --user postgres --password yourpassword --csv bengaluru_roads_master.csv > load.log 2>&1
```
...
Backend services should connect using standard PostgreSQL connection parameters (host, port, dbname, user, password), typically loaded from environment variables rather than hardcoded:
